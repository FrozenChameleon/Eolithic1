...
Resource* BmFontManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = BmFont_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void BmFontManager_LoadAllFromDat()
{
	...
	while (DatReader_HasNext(dr))
	{
		...
		BmFontManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
