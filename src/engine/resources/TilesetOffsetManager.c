...
#include "TilesetOffsetManager.h"
...
TilesetOffset* TilesetOffsetManager_GetResourceData(const char* filenameWithoutExtension)
{
	...
	Resource* resource = TilesetOffsetManager_GetResource(filenameWithoutExtension);
	if (resource == NULL)
	{
	...
	return (TilesetOffset*)resource->mData;
}
Resource* TilesetOffsetManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = TilesetOffset_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void TilesetOffsetManager_LoadAllFromDat()
{
	...
	MString* path = File_Combine2("data", TilesetOffsetManager_GetDatFileName());
	...
	while (DatReader_HasNext(dr))
	{
		...
		TilesetOffsetManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
...
void TilesetOffsetManager_Dispose(const char* filenameWithoutExtension)
{
	...
	{
		TilesetOffset_Dispose(resource->mData);
	}
	Utils_free(resource);
...
void TilesetOffsetManager_DisposeAll()
{
	...
	{
		TilesetOffsetManager_Dispose(sh_resources[i].key);
	}
...
{
	return (TilesetOffset*)sh_resources[index].value->mData;
}