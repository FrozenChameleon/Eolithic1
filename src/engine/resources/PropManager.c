...
Resource* PropManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = Prop_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void PropManager_LoadAllFromDat()
{
	...
	MString* path = File_Combine2("data", PropManager_GetDatFileName());
	...
	while (DatReader_HasNext(dr))
	{
		...
		PropManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
...
void PropManager_DisposeAll()
{
	...
	{
		PropManager_Dispose(sh_resources[i].key);
	}
...
{
	return (Prop*)sh_resources[index].value->mData;
}