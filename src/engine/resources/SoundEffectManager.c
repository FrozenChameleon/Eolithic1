...
void SoundEffectManager_LoadAllFromDat()
{
	...
	while (DatReader_HasNext(dr))
	{
		...
		SoundEffectManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
...
int64_t SoundEffectManager_Length()
{
...
{
	return (SoundEffect*)sh_resources[index].value->mData;
}