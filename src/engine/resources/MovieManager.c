...
Movie* MovieManager_GetResourceData(const char* filenameWithoutExtension)
{
	...
	Resource* resource = MovieManager_GetResource(filenameWithoutExtension);
	if (resource == NULL)
	{
...
Resource* MovieManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = Movie_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void MovieManager_LoadAllFromDat()
{
	...
	MString* path = File_Combine2("data", MovieManager_GetDatFileName());
	...
	while (DatReader_HasNext(dr))
	{
		...
		MovieManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
...
void MovieManager_DisposeAll()
{
	...
	{
		MovieManager_Dispose(sh_resources[i].key);
	}
...
{
	return (Movie*)sh_resources[index].value->mData;
}