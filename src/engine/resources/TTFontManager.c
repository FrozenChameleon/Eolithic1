...
TTFont* TTFontManager_GetResourceData(const char* filenameWithoutExtension)
{
	...
	Resource* resource = TTFontManager_GetResource(filenameWithoutExtension);
	if (resource == NULL)
	{
...
Resource* TTFontManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = TTFont_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void TTFontManager_LoadAllFromDat()
{
	...
	MString* path = File_Combine2("data", TTFontManager_GetDatFileName());
	...
	while (DatReader_HasNext(dr))
	{
		...
		TTFontManager_LoadAssetFromStreamAndCreateResource(br, MString_GetText(fileNameWithoutExtension), MString_GetText(path));
...
void TTFontManager_DisposeAll()
{
	...
	{
		TTFontManager_Dispose(sh_resources[i].key);
	}
...
{
	return (TTFont*)sh_resources[index].value->mData;
}