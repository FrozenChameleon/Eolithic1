...
MovieTiming* MovieTimingManager_GetResourceData(const char* filenameWithoutExtension)
{
	...
	Resource* resource = MovieTimingManager_GetResource(filenameWithoutExtension);
	if (resource == NULL)
	{
	...
	return (MovieTiming*)resource->mData;
}
Resource* MovieTimingManager_LoadAssetFromStreamAndCreateResource(BufferReader* br, const char* filenameWithoutExtension, const char* path)
{
	...
	_mResourceCounter += 1;
	resource->mData = MovieTiming_FromStream(MString_GetText(resource->mPath), MString_GetText(resource->mFileNameWithoutExtension), br);
...
void MovieTimingManager_LoadAllFromDat()
{
	...
	MString* path = File_Combine2("data", MovieTimingManager_GetDatFileName());
...
void MovieTimingManager_Dispose(const char* filenameWithoutExtension)
{
	...
	{
		MovieTiming_Dispose(resource->mData);
	}
	Utils_free(resource);
...
void MovieTimingManager_DisposeAll()
{
	...
	{
		MovieTimingManager_Dispose(sh_resources[i].key);
	}
...
{
	return (MovieTiming*)sh_resources[index].value->mData;
}